...
async function main(): Promise<ScriptObservation[]> {
    ...
    try {
        ...
        const whoami = (whoami_res.status === "fulfilled" ? whoami_res.value : {}) as WhoamiResponse;
        const credits_data = credits_res.value as CreditsResponse;
        const sub_data = (sub_res.status === "fulfilled" ? sub_res.value : {}) as SubscriptionsResponse;
        ...
        const account_label = user_name
            ? (plan_tag ? `${user_name} (${plan_tag})` : user_name)
            : "Command Code";
        ...
        const monthly_remaining =
            credits_data.credits?.monthlyCredits !== undefined
                ...
                : null;
        const monthly_cap =
            plan_id && PLAN_MONTHLY_CAP[plan_id] !== undefined
                ? PLAN_MONTHLY_CAP[plan_id]
                : null;
