...
### 应用与系统级图标规范（Dock 与菜单栏）

遵循 Apple Human Interface Guidelines 与跨平台托盘标准：
...
内容卡片 = `card` token：14px 圆角 + 发丝描边 + `shadow-card` + 16px 内边距。卡片标题用 `title-md`，右上角可挂卡片菜单（`z-[var(--z-menu)]` 层）。骨架屏（`skeleton`）用 shimmer 渐变，与卡片同圆角同布局。
