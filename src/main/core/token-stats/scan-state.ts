...
export interface SerializedScanState {
    ...
    commandcode_states?: Record<string, SerializedScanBucket>;
...
export interface ScanStateMaps {
    ...
    readonly commandcode_states: Map<string, CommandCodeScanState>;
...
export function serialize_state(maps: ScanStateMaps): SerializedScanState {
    ...
    const commandcode: Record<string, SerializedScanBucket> = {};
    for (const [key, state] of maps.commandcode_states) {
        commandcode[key] = serialize_bucket(state);
    }
    ...
    return {
        ...
        grok_states: grok,
        codex_states: codex,
        commandcode_states: commandcode,
...
export async function load_state(
    ...
    try {
        ...
        if (s.commandcode_states) {
            for (const [k, bucket] of Object.entries(s.commandcode_states)) {
                maps.commandcode_states.set(
                    k,
                    deserialize_bucket(bucket) as unknown as CommandCodeScanState,
                );
            }
        }
