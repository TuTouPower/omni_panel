...
- **开发面板通路（t481）**：桌面 `devPanel:*` IPC 与 Web `/v1/devPanel/{scan,status,cancel}` 都调用同一个主进程 `DevPanelScanManager`；配置落入 `AppConfiguration.devPanel`，Git 命令经无 shell `execFile` 只读执行。结果携带 `scanned_at` 与单调 `data_version`，重复扫描请求复用在途任务。
