...
- **持久定位索引（t254）**：`session-locator` 的解析结果持久化到 `<dataRoot>/session-path-index.json`（`session-path-index.ts`），跨重启命中免整目录递归扫描；命中校验 `mtime_ms + size` + `paths_key` 签名（防跨配置命中旧路径）。失效回退扫描并修正索引；写盘失败仅记日志跳过（回退扫描）。WSL 用户名探测结果进程内缓存 + 随索引跨重启缓存，探测失败（空串）不写负缓存下次重探测自愈。
...
- **会话库查询路径（t227，t248）**：main 侧 `query_session_stats` 独立聚合全量会话数、Agent 数、tokens 与 source 计数；`query_sessions` 按 `sources[]`/`search`/`start_at`/`end_at`/`order_by`/`direction` 做白名单 SQL 过滤与分页，renderer 只持有当前已加载页；内容搜索合并后端元信息命中与正文命中，摘要只请求当前可见会话。
...
- **web 认证链路（t278）**：local-api 新增 grok/kimi OAuth 六端点（login_start / login_poll / status / logout / refresh）与 cookie 登录触发/状态组（`/v1/auth/cookieLogin`、`/v1/auth/cookieLogin/status`）；web bridge（`src/web/usageboard-web.ts`）与 preload 同步暴露，设置页 device-code 在页面内展示 URL+码并轮询，cookie 登录为立即触发 + `cookieLoginStatus` 轮询（250ms/120s 超时）。cookie 捕获复用 session-manager 隔离 partition 的可见 BrowserWindow（CLI/桌面同代码路径），捕获成功后密钥落 vault、web 编辑路径重读 secrets 并刷新 connector；无 display 时在创建窗口前返回可读错误，设置页提供手动粘贴 Cookie 回退（与自动捕获共用 secrets 保存链路）。认证全流程日志脱敏（cookie/token 不落日志，开发期同样生效）。已知降级：web/headless 下无静默 cookie 续期，过期需重新登录或重新粘贴。
- **web 实时推送（t279）**：SSE 通道（`GET /v1/events`）在 runtimeStore state 消息外扩展会话历史订阅事件 `messagesUpdated`。订阅经专属事件流注册：bridge 以自增 `subscriber_id` 查询参数开 EventSource，连接建立（`open`）后 POST `/v1/sessionHistory/subscribe` 注册（同 id 幂等，重复只换 on_update），服务端把 watcher 增量只推给持有该订阅的连接；`unsubscribe` 或 SSE 连接关闭（cleanup 带映射/client 双身份校验，防重连竞态误删新注册）均注销订阅并停 watcher，杜绝泄漏。断连自动重连在 `open` 时以同 id 重挂，实时推送不丢失；注册/重挂失败由 renderer 5s 轮询兜底。日志导出：`GET /v1/logs/export` 流式输出当前活跃日志段（`<userData>/logs/app-<date>.log`，chunked 无 Content-Length），`Content-Disposition` 触发浏览器下载，web bridge `logs.export` 与桌面保存对话框语义对齐。
