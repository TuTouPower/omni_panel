...
- **会话库视图（t227，t248，t439，t458）**：「会话库」页签由 `SessionLibrary`（`src/renderer/components/session-library/`）渲染：搜索（默认元信息 +「包含消息内容」开关并集正文搜索，后端候选筛选分页、序号守卫防迟到覆盖；t458 起筛栏另有独立「标题」「工作目录」输入，值作为查询 `title`/`directory` 与其余条件 AND，空值不带参数，清空筛选一并清空，内容搜索候选同样受其约束）、agent 多选、四排序、网格/列表、加载更多分页、预览抽屉（前 5 条）、SelectionDock 批量打开（复用摘选 store 与槽位模型）。SelectionDock「并排打开」为替换语义（t439）：经 `SessionShell` 下传的 `clear_workspace_ref` 先调用 `WorkspaceView` 注册的 `clear_all` 清空全部槽位，再逐个 `sessionHistory.open`（与 `confirm_recent` 同序），单独打开仍为装入/追加。筛选/排序为纯函数 `lib/session-library/filter.ts`；勾选身份用 `source|env|id` 主键；首条用户消息摘要与内容搜索经 session-history 批量接口读源文件消息（只读）。
- **会话库查询路径（t227，t248，t457）**：main 侧 `query_session_stats` 独立聚合全量会话数、Agent 数、tokens 与 source 计数；`query_sessions` 按 `sources[]`/`search`/`title`/`directory`/`start_at`/`end_at`/`order_by`/`direction` 做白名单 SQL 过滤与分页——t457 起 `title`/`directory` 为独立条件（大小写不敏感子串、与既有条件全 AND、空/省略不约束），HTTP `/v1/sessions`、桌面 `TOKEN_STATS_SESSIONS`、web `getSessions` 三入口收敛到同一过滤语义；renderer 只持有当前已加载页；内容搜索（searchContent）filters 同样接受 `title`/`directory`，候选枚举与 metadata 查询两路径均透传后合并元信息命中与正文命中，摘要只请求当前可见会话。
