...
## 035 连接器网络响应体单项上限收敛至 10MB（2026-09-25）

- 背景：原 `net-client.ts` 响应体硬限制设为 50MB，高并发刷新场景（如多账号同时收集）可能瞬间在 Node 堆中产生数百兆常驻内存放大；恶意或异常上游返回超大 payload 易引发 OOM。
- 结论：将单次 HTTP 响应体读取上限由 50MB 下调至 10MB（`MAX_RESPONSE_BYTES = 10 * 1024 * 1024`）。在流读取过程中执行实时字节统计，达到阈值立即调用 `body.destroy()` 中断底层连接，避免无谓网络传输与内存全量拼接。错误响应体限制保持在 1MB，并保留前 500B 脱敏片段用于诊断。
- 落地：t514。
- 替代：保持 50MB 上限；读取完成后再抛错。

## 036 连接器隔离进程执行与内置完整性校验（2026-09-25）

- 背景：原 `node:vm` 运行时不防范原型链逃逸与恶意攻击，用户外部目录连接器可能造成主进程 RCE 致命安全隐患。
- 结论：
    1. 连接器执行迁移至独立的子进程/utilityProcess 隔离环境，其崩溃、内存溢出或死循环被父进程超时强行终结，完全不拖垮主进程。
    2. 加载前通过 SHA-256 完整性清单对内置连接器逐一比对，哈希不匹配直接拒绝加载并报安全警报。
