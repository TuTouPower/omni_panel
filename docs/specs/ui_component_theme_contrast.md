...
- 自定义字号类须用显式 `text-[length:var(--text-*)]`（d032 机制，全仓 `cn()` 组合与 className 统一），避免 tailwind-merge 把自定义字号 token 误判为颜色类、吞掉同组的 `text-[var(--color-on-*)]` 颜色声明。初始落地：Button（t298）+ ui/会话侧组件全仓统一（t302）。
