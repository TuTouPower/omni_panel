...
> 窗口外壳（t223）与工作台槽位模型（t224）：route `history` 现由单壳双页签外壳承载（`session-shell.md`），工作台页签内为 8 槽位模型（`workspace.md`）。本 spec 描述被 t224 取代前的 6 栏会话视图行为，保留作能力来源参考（消息渲染/推送/分页/选择/复制语义仍生效）。
