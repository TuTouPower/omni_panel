...
- **毛玻璃**：`backdrop-blur` 仅菜单类浮层与对话框遮罩；SelectionBar 等非菜单浮层用实底，禁止 blur。
...
|SelectionBar|`z-20`+blur → `z-sticky`+`surface-window` 实底|
...
- 三同类位点裸类换任意值（数值不变）：`SessionPane` 回底钮 `z-sticky`→`z-[var(--z-sticky)]`、大纲抽屉 `z-context`→`z-[var(--z-context)]`、`SelectionBar` `z-sticky`→`z-[var(--z-sticky)]`；RangePicker 本体行归 t451。
