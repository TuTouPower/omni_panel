...
- t438：linux 宿主 win 源 resolve 经 `win_home_wsl`（`/mnt/c/Users/<u>` 自动发现，与 collector 同源 `win-home-discovery.ts`）。resolve 时惰性发现（非启动一次性注入，直接调 `resolve_session_file` 的路径同样生效）；显式字符串优先，`""` = 禁用哨兵；成功结果进程内缓存，失败（null）负缓存 60s 时间窗节流（防零候选画像下每次 resolve 同步 spawn powershell.exe），窗后重探自愈。`paths_key` 签名改用 effective `win_home_wsl`——发现结果变化 → 旧索引条目不命中 → 回退扫描重建。
...
- `tests/unit/main/core/session-history/session-locator.test.ts`：t438 新增 win 源经 `win_home_wsl` 定位（kimi/claude）、缺省惰性发现、`""` 禁用哨兵、发现失败负缓存节流与窗后自愈、发现结果变化 → `paths_key` 变化 → 旧索引条目失效重扫；全文件默认装 null probe 保 hermetic。
