> P6：工作台不再作为用户面入口；会话打开改为会话库 + 同屏查看（见 `session-shell.md`）。本文保留槽位模型实现参考。
...
- **session-grid 间隙（t411）**：`.session-grid` 用透明 `gap-[var(--spacing-card-gap)]`（token 12px），去掉 `gap-px` + `bg-outline` + `p-px` 描线网格；间隙透出窗口底色，行列交汇处不再露星形洞。卡片保留自身描边与圆角；列数/行数与 `content-start` 策略不变。
...
- rail 头部行内含槽位计数与折叠钮（t413，无独立 `session-rail-toggle-row`）；底部固定「添加会话」（展开文字/折叠态加号），满槽 disabled；rail 可折叠/展开。
...
- 结果区：网格卡片（按内容宽度自适应，最小列宽 280px，最多 5 列；不足单列宽度时收缩）或列表行；卡片对齐 demo 三行结构——行1 agent 徽标 + 首条→末条消息时间区间（紧凑格式，悬停精确时间）+ 悬停浮现勾选框（已选常显）；行2 目录末级名 · tokens（半粗）· N 轮 · 右端 IdChip（点击复制完整 session id，短显 8 位，成功「已复制」1.4s 还原）；行3 末条用户消息摘要（`summaries` mode=last 懒加载，无标题行）；顶部 agent 色条保留；hover 浮现「单独打开/预览」；点卡片/行勾选（上限 8）。
