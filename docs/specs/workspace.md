...
- 侧边栏：搜索（元信息 / 含消息内容）、时间预设（全部/24h/7 天/30 天 + 自定义弹层）、agent logo 行多选（数量角标）、Token 数/轮次数轴区间（双滑杆，含边界，拖至端点即不限，上限来自全量统计 max_tokens/max_calls，防抖 300ms 提交后端）、排序、视图、「同屏最近 2/4/6/8」（替换语义并排打开）、重置。
- 结果区：网格卡片（agent 色条/徽标/标题/首条用户消息摘要懒加载/meta 轮数·tokens·首条→末条消息时间区间/目录）或列表行；hover 浮现「单独打开/预览」；点卡片/行勾选（上限 8）。
