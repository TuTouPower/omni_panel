...
- **自定义范围交互**（t451）：时间范围下拉选「自定义」稳定打开日期面板（含已处自定义态重选可重开；打开期间下拉保持「自定义」）；非法区间行内报错且面板不关闭；编辑中途后台刷新不重置输入；自定义区间随视图偏好持久化，刷新后恢复并沿用查询。（t453：click 重开后无选择变更而失焦自动关闭。t454：入口收敛为下拉单入口，「📅 自定义」按钮删除；开下拉动作与选中动作以有无 mousedown 区分，仅后者开面板。）
