...
- 组件形态全集与各形态 token 取值以 DESIGN.md `components` 节为真相源。取值规范（t301 六项对齐结果）：
    ...
    - **Segmented**：容器 `surface-raised` + 选中块 `surface-card`/`primary`/`shadow-card`（t435：对齐 DESIGN `segmented-item-active`，此前误用 `surface-window`/`on-surface`）。t421：option 支持 `title` / `aria-label` / `data-testid`；会话库视图、Provider 概览/明细、趋势窗口分段统一消费本组件，档外圆角（`rounded-[7px]`/`[9px]` 分段）收敛。
