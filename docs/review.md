...
### 12. Dashboard/Popup 没显示上次更新时间，失败状态也丢掉 lastSuccess 内容
...
IPC 在 failed DTO 中会携带 `lastSuccess` 的 `items` 和 `updatedAt`，但 `PluginCard` failed 分支只显示错误，不显示缓存数据。ready 分支也没有显示 `updatedAt`。

这削弱了“失败保留上次成功 cache”和“上次更新时间”的用户价值。建议 failed 时展示错误 banner + stale 数据，ready 时展示更新时间或相对时间。
...
`handleConfigSave()` 只做 schema 结构校验，然后接受 renderer 传来的完整 `AppConfiguration`。`executablePath` 只要求非空字符串，未验证是否来自已发现插件或允许目录；refresh interval 也只要求 `>= 1` 秒，没有 Settings 需求里的 1–60 分钟业务范围。

考虑到 renderer 是信任边界外侧，这里建议至少：

- 只允许修改已存在 instance 的可编辑字段。
- executablePath / stateId / instanceId 不由 renderer 任意改。
- refresh interval 由 main 进程 enforce 业务范围。
...
插件卡片只显示插件名和 `used / limit`，缺少关键上下文：

- 当前状态：idle / loading / ready / failed 没有足够明显的视觉区分。
- 更新时间：`updatedAt` 没有展示，用户无法判断数据新鲜度。
- 额度语义：没有剩余额度、百分比文本、接近上限警告。
- badge / chart：IPC 和 schema 支持这些字段，但 UI 基本没有利用。

结果是 Dashboard 不能承担“监控”职责，只像一个简陋结果列表。

建议：

- 每张卡片显示状态徽章、最后刷新时间、使用百分比和剩余额度。
- 接近额度上限时用明确 warning 颜色。
