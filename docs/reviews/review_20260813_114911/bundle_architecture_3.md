...
- [Medium][55] scripts/token-stats-spike.ts:1 — 遗留 Phase 0 spike 长期驻留 scripts/，硬编码个人环境 — 该文件自 2026 单次提交（302e509f）后从未更新，仍含硬编码 `wsl_distro = "Ubuntu-22.04"`、`wsl_user = "karon"`、UNC 路径（:17-21）与文件级 `eslint-disable`（:1）；其结论已被 token-stats 正式实现（readers/collector）取代，作为历史产物放在 scripts/ 会误导新读者（误以为当前数据源仍走这些路径）。修复建议：迁至 `docs/spikes/{sid}/code/`（项目约定 spike 产物位置）或标注已归档；如保留运行入口需参数化 distro/user。
