...
- 根因：产品缺陷(跨平台采集支持缺失)。collector 路径构建硬编码 Windows 宿主假设:env 仅 win/wsl 两档无本机维度;`win_home=homedir()` 在 Linux 返回 POSIX home,win 源拼接成 `/home/testuser\.claude\projects`(反斜杠字面量)不存在;wsl 源 `\\wsl.localhost\{distro}\home\{user}`(UNC)在 Linux 内不可达,`effective_wsl_user` 探测失败返回空串;ENOENT 静默,无告警。从 8-09 05:36 后持续全 0。
