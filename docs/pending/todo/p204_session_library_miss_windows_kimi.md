...
- 现象：期望在会话库搜「黑沙皇」（勾选「包含消息内容」）命中 Windows Kimi 会话 `session_e36b69aa-2e48-4d19-8494-9ed890da3612`（cwd `D:/Kar/Code`，wire 含该词 318 次）。实际 OmniPanel 在 **WSL 跑 + 网页版搜索** 时结果空。同机 wire 按 extractor 规则模拟提取 99 条消息、关键词命中 25 条——正文匹配本身成立。未勾选「包含消息内容」时 UI 未说明搜的是 title/directory/id（不是只搜 cwd）。
...
- 根因（产品缺陷）：
    ...
    2. WSL 宿主 `kimi_local` 经 `paths.kimi_sessions_path(..., env=local)` 落到 `os.homedir()/.kimi-code`（本机 `/home/karon/.kimi-code`），**不会**读 `C:\Users\Karson\.kimi-code`。
