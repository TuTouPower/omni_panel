...
- 影响：任何在本会话（或同类 agent shell）里跑 renderer 项目（`tests/unit/renderer/**`、`tests/smoke/**`、`tests/unit/web/**`）的 task 都会把该失败误判为仓库缺陷并写进 handoff/notes；反向地，若据此认定「renderer 不可测」会漏掉真实回归。
- 现状：有效
