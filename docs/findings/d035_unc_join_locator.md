...
- 证据：`node -e` 实测——`path.win32.join('\\\\wsl.localhost\\Ubuntu-22.04','home','testuser','.claude','projects')` → `\\wsl.localhost\Ubuntu-22.04\home\testuser\.claude\projects`；空用户名 → `\\wsl.localhost\Ubuntu-22.04\home\.claude`。Windows 真机留 `[deploy]`。
