...
- 证据：`node -e` 实测 `platform: linux`、`path.join(os.homedir(), '.claude', 'projects')` → `/home/testuser/.claude/projects`；`path.win32.join('\\\\wsl.localhost\\Ubuntu','home','testuser','.claude','projects')` → `\\wsl.localhost\Ubuntu\home\testuser\.claude\projects`。Windows 真机行为留 `[deploy]`。
