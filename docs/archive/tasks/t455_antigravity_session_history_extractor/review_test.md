...
落点校验：不带 `-C` 的 `git rev-parse --show-toplevel`（工作目录 `/home/karon/karson_ubuntu/omni_panel_t455`）输出去空白后精确等于 `/home/karon/karson_ubuntu/omni_panel_t455`，通过。

diff 审查范围：`git -C '/home/karon/karson_ubuntu/omni_panel_t455' diff 284914af9dec4051c88d142ebe9a03903ce10f61`，测试改动仅新增一个文件 `tests/unit/main/core/session-history/antigravity-extractor.test.ts`（5 个 it），无既有测试修改/删除。`spec.md` 上下文区「未知契约清单」已无 `UNVERIFIED-BLOCKING` / `UNVERIFIED-SPIKE` / 裸 `UNVERIFIED`（两项均标已验证），门禁项通过。
