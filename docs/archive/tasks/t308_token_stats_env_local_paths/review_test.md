...
- AC-002：`re_verified`。`paths.test.ts`「windows host」describe 断言 win_home 反斜杠路径与 `\\wsl.localhost\Ubuntu-22.04\home\testuser\...` UNC（`toBe` 精确匹配，已跑通过）；`collector.test.ts` `set_collector_host("windows")` 下 local sources 走 `C:\Users\Test`（`stringContaining("Users")`）
...
- AC-002：`re_verified` — paths.test.ts windows host win_home 反斜杠路径与 `\\wsl.localhost\Ubuntu-22.04\home\testuser\...` UNC 精确 `toBe`；collector.test.ts `set_collector_host("windows")` 下 local 源走 `C:\Users\Test`
