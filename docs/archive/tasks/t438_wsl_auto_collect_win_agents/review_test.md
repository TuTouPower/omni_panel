...
- 建议：补一条测试——`set_win_home_wsl_probe(() => null)` 后 `configure`，断言 win 源 unavailable；再 `set_win_home_wsl_probe(() => "/mnt/c/Users/TestUser")` 并再次 `collect`，断言 probe 被再次调用、win 源转 ok。
