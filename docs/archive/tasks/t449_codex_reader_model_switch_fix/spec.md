...
t448 已修 codex reader 重复 total 事件 double 计，但残留缺陷：`turn_context.model` 变化时 reader 把差分基准 `segment_prev_total`/`segment_prev_cache` 重置，下个 token_count 按全量计入。codex `total_token_usage` 实测为文件级连续单调累计（116 文件 0 回绕，含 model 切换处无跳变），不随 model 重置——双 model 会话在切换点整段双计。真实 omni_game gpt-5.6-sol 会话入库 tokens 392M（应 196M）。p216。
...
- [ ] AC-002：真实 omni_game gpt-5.6-sol 文件（948 事件 2 切换）重扫后入库 tokens 回落约 196M 量级（读库或 reader 输出，允许百分位误差）。
...
- 来源：p216（2026-09-04 实测：omni_game gpt-5.6-sol 会话入库 392M vs 文件累计 196M；116 文件 0 回绕含 model 切换处无跳变）
