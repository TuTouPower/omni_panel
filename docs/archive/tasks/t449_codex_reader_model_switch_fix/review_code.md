...
- AC-002：`trust_prior`。真实 omni_game gpt-5.6-sol 文件（`.codex/sessions`）仓库外黑盒数据，本 worktree 不可达；依赖 spec 可测试性声明的等价自造双 model fixture 覆盖与 mtime-incremental 重扫入库机制证据。
