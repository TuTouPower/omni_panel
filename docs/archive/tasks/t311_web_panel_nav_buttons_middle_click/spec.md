...
web 端（浏览器连 local-api）右上角面板互跳按钮、Usage 标题栏设置/代理面板按钮、设置-关于页外链卡片、Usage 空态“添加服务”入口均无法鼠标中键/Ctrl+Click 新开标签页。根因：web 端导航/外链语义实现为 `<button onClick>` + JS 改 hash（最终路由为 `setting`/`agent`/`session`/`usage`）或 `window.open`；浏览器仅对原生 `<a href>` 支持中键/Ctrl+Click 新开标签页。p137 核实确认 4 个同因位点。
...
- 左键点击后目标路由切换行为与现状一致（`#setting`/`#agent`/`#session`/`#usage` 各自路由挂载不变）。
...
- AC-001/002/003/005/006：可用组件测试断言 web 分支渲染 `<a>` 的 href/target/rel，桌面分支仍为 `<button>`；web e2e 验证左键 hash 切换。
