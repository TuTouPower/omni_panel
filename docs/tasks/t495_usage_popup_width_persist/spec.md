...
- [ ] AC-001：用户拖动改变 usage popup 宽度后，隐藏再显示保持该宽度。
- [ ] AC-002：重启应用后首次显示 usage popup 使用上次保存的宽度。
- [ ] AC-003：保存/恢复的宽度被 clamp 到 `[USAGE_MIN_WIDTH, 所在显示器的 workArea.width]`。
- [ ] AC-004：保存的宽度超过当前显示器工作区（如换到更小分辨率/副屏）时，显示宽度被钳制到可见范围内。
- [ ] AC-005：配置中无该键（旧配置）时按现状默认宽度 482 显示，不报错。
- [ ] AC-006：popup 的锚定位置与动态高度行为不回归（每次显示仍贴近托盘、内容高度仍自适应）。
