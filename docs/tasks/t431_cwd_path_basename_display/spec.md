...
前端 demo 会话卡片 cwd 显示完整路径（如 /home/karson/karson_ubuntu/repo_template）过长，用户要求只显示最后一段目录名（repo_template）。改动组件 `public/frontend_demo/app/src/components/CwdPath.tsx`，全局 4 处使用（SessionCard/SessionPane/RecentSessionsModal/SessionPickerModal）同步生效。注意这是 frontend_demo（demo 应用），非 src/renderer 生产树。
...
- [ ] AC-001：CwdPath 对完整路径只渲染最后一段目录名（如 /home/karson/karson_ubuntu/repo_template → repo_template）。
