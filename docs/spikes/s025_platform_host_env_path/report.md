...
- `platform: linux`（本机）；`sep: /`；`join(os.homedir(), '.claude', 'projects')` → `/home/karon/.claude/projects`。
- `path.win32.join('C:\\Users\\test', '.claude', 'projects')` → `C:\Users\test\.claude\projects`。
- `path.win32.join('\\\\wsl.localhost\\Ubuntu', 'home', 'karon', '.claude', 'projects')` → `\\wsl.localhost\Ubuntu\home\karon\.claude\projects`。
