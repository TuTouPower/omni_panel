...
- `path.win32.join('\\\\wsl.localhost\\Ubuntu-22.04','home','karon','.claude','projects')` → `\\wsl.localhost\Ubuntu-22.04\home\karon\.claude\projects`（UNC 前缀保留，正常拼接）。
- `path.join('/home/karon','.claude','projects')` → `/home/karon/.claude/projects`（POSIX 正常；`\` 字面量在 POSIX 中是普通字符，拼接 `C:\...` 会产出坏路径）。
