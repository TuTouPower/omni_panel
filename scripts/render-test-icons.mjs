...
function get_test_tray_template_svg(raw_svg) {
    ...
    s = s.replace(
        /(<defs>[\s\S]*?<\/defs>)([\s\S]*)(<\/svg>)/,
        `$1<g transform="translate(512, 512) scale(${824 / 780}) translate(-512, -512)">$2${t_badge}</g>$3`,
    );
    return s;
}
...
// 1. macOS / Linux Test Dock/App icon (1024x1024 with 824x824 body)
...
const tray_png = render_png(SVG, TRAY_SIZE);
...
// 4. macOS Test Tray Template Icons (16x16 @1x, 32x32 @2x, pure black + alpha)
const test_template_svg = get_test_tray_template_svg(SVG);
const test_tray_1x = render_png(test_template_svg, 16);
const OUT_TEST_TRAY_1X = resolve(ROOT, "assets/tray-icon-testTemplate.png");
writeFileSync(OUT_TEST_TRAY_1X, test_tray_1x);
console.log(`[render-test-icons] wrote ${OUT_TEST_TRAY_1X} (16x16)`);

const test_tray_2x = render_png(test_template_svg, 32);
const OUT_TEST_TRAY_2X = resolve(ROOT, "assets/tray-icon-testTemplate@2x.png");
writeFileSync(OUT_TEST_TRAY_2X, test_tray_2x);
console.log(`[render-test-icons] wrote ${OUT_TEST_TRAY_2X} (32x32)`);
