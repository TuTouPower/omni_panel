...
describe("discover_win_home (t438)", () => {
    ...
    it("② 唯一候选即用", () => {
        const d = deps(["All Users", "Default", "Default User", "Public", "Karson"], {
            Karson: [".kimi-code", ".claude"],
        });
        expect(discover_win_home(d)).toBe("/mnt/c/Users/Karson");
    });

    it("系统项（All Users/Default/Default User/Public）不参与候选", () => {
        // 即使系统项带标记目录也不选（它们不是真实用户目录）。
        const d = deps(["All Users", "Public", "Default", "Default User", "Karson"], {
            "All Users": [".claude"],
            Public: [".grok"],
            Default: [".kimi-code"],
    ...
    it("④ 零候选回退 shell 探测 $env:USERPROFILE（含 CRLF 与尾部空白）", () => {
        const d = deps(["All Users", "Default", "Public"], {}, "C:\\Users\\Karson\r\n", [
            "/mnt/c/Users/Karson",
        ]);
        expect(discover_win_home(d)).toBe("/mnt/c/Users/Karson");
    });
    ...
    it("④ shell 返回小写盘符也转换", () => {
        ...
        const d = deps(["All Users", "Default"], {}, "c:\\users\\karson", ["/mnt/c/users/karson"]);
        expect(discover_win_home(d)).toBe("/mnt/c/users/karson");
    });
    ...
    it("⑤ shell 也失败（exec 返回空）→ null", () => {
        ...
        expect(discover_win_home(deps([], {}, "C:\\Users\\Karson\r\n"))).toBeNull();
    });
