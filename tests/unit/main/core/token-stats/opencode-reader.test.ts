...
describe("read_opencode_sessions", () => {
    ...
    it("reads sessions with valid tokens", () => {
        ...
        const { sessions } = read_opencode_sessions(db_path, "linux", 0);
        expect(sessions).toHaveLength(2);
        expect(sessions[0]!.id).toBe("sess-001");
        expect(sessions[0]!.source).toBe("opencode");
        expect(sessions[0]!.env).toBe("linux");
    ...
    it("derives daily usage rows from step-finish part tokens", () => {
        ...
        const { sessions, daily, records } = read_opencode_sessions(db_path, "linux", 0);
    ...
    it("skips assistant messages without step-finish parts", () => {
        ...
        const { sessions, daily } = read_opencode_sessions(db_path, "linux", 0);
    ...
    it("filters by max_updated (incremental)", () => {
        ...
        const { sessions } = read_opencode_sessions(db_path, "linux", 3000);
        expect(sessions).toHaveLength(1);
    ...
    it("returns empty array for missing database", () => {
        const { sessions } = read_opencode_sessions("/nonexistent/path/db.sqlite", "linux", 0);
