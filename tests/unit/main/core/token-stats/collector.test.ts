...
describe("collector", () => {
    ...
    describe("path builders (t308: host-injected)", () => {
        it("builds env=win paths from win_home on a windows host", () => {
            expect(claude_costs_path(base_config, "win", "windows")).toBe(
                "C:\\Users\\Test\\.claude\\metrics\\costs.jsonl",
            );
            expect(claude_projects_path(base_config, "win", "windows")).toBe(
                "C:\\Users\\Test\\.claude\\projects",
            );
            expect(opencode_path(base_config, "win", "windows")).toBe(
                "C:\\Users\\Test\\.local\\share\\opencode\\opencode.db",
            );
            expect(kimi_sessions_path(base_config, "win", "windows")).toBe(
                "C:\\Users\\Test\\.kimi-code\\sessions",
            );
            expect(kimi_index_path(base_config, "win", "windows")).toBe(
                "C:\\Users\\Test\\.kimi-code\\session_index.jsonl",
            );
        });

        it("builds env=linux/mac POSIX paths from homedir on a non-Windows host (AC-001)", () => {
            expect(claude_costs_path(base_config, "linux", "linux", "/home/u")).toBe(
                "/home/u/.claude/metrics/costs.jsonl",
            );
            expect(claude_projects_path(base_config, "linux", "linux", "/home/u")).toBe(
                "/home/u/.claude/projects",
            );
            expect(opencode_path(base_config, "linux", "linux", "/home/u")).toBe(
                "/home/u/.local/share/opencode/opencode.db",
            );
            expect(kimi_sessions_path(base_config, "linux", "linux", "/home/u")).toBe(
                "/home/u/.kimi-code/sessions",
            );
            expect(kimi_index_path(base_config, "linux", "linux", "/home/u")).toBe(
        ...
        it("t426: resolves linux/mac grok sessions path on non-Windows hosts (AC-002)", () => {
            expect(grok_sessions_path(base_config, "linux", "linux", "/home/u")).toBe(
                "/home/u/.grok/sessions",
            );
            expect(grok_sessions_path(base_config, "mac", "macos", "/Users/u")).toBe(
    ...
    describe("collect()", () => {
        ...
        it("re-emits records when postMessage fails (t345 AC-004 / t393 AC-001)", () => {
            ...
            mock_scan_jsonls.mockImplementation(() => {
                if (jsonl_states.has("claude_jsonl_win")) {
                    return {
                        sessions: [],
                        daily: [],
                        records: [],
            ...
            expect(jsonl_states.has("claude_jsonl_win")).toBe(false);
        ...
        it("skips WSL sources when wsl_enabled=false", () => {
            ...
            // t426: grok 现在有平台源（随宿主安装），wsl_enabled=false 只跳过
            // grok_wsl；grok_win 在 windows 宿主读 win_home\.grok（t197 断言更新）。
            expect(mock_scan_grok).toHaveBeenCalledTimes(1);
            expect(String(mock_scan_grok.mock.calls[0]![0])).toContain("Users");
            expect(mock_scan_grok.mock.calls[0]![1]).toBe("win");
        ...
        it("t426: host=linux source list includes local grok and collects it (AC-001)", () => {
            ...
            expect(
                update.sessions.some(
                    (s) => s.id === "grok-linux-s1" && s.env === "linux" && s.source === "grok",
                ),
            ).toBe(true);
            expect(update.records.some((r) => r.message_id === "grok-linux-r1")).toBe(true);
        });

        it("reads the grok wsl source under WSL and posts its rows (t197)", () => {
            // t426: grok 双源——windows 宿主上 grok_win 读 win_home（通常缺失），
            ...
            // 两个 grok 源都参与：grok_win（win_home）+ grok_wsl（UNC）。
        ...
        it("warns once when the grok sessions dir is missing and still collects others (t197 AC5)", () => {
            // t426: grok 双源——wsl_config + windows host 下 grok_win（win_home）
            ...
            // token_stats_update + two collector_log warns (grok_win + grok_wsl missing).
            ...
            const log_msgs = mock_post_message.mock.calls
                ...
                .map(
                    (c) => c[0] as { type: string; level: string; module: string; message: string },
                );
            ...
            expect(log_msgs.some((l) => l.message.includes("grok_win sessions dir missing"))).toBe(
                true,
            );
        ...
        it("advances the truncation cursor across rounds so no session is lost (t345 AC-003 + t385 AC-001)", () => {
            ...
            expect(update.sessions).toHaveLength(10000);
            const cursor = source_cursors.get("claude_costs_win");
            ...
            expect(source_cursors.has("claude_costs_win")).toBe(false);
        });
        ...
        it("t385 AC-002: 截断期间新会话排序在游标前仍被扫描入列（身份键非位置计数）", () => {
            ...
            expect(update.sessions).toHaveLength(10000);
            // 首轮游标 = {s0..s9999}（身份键）
            expect(source_cursors.get("claude_costs_win")?.sessions.has("s0")).toBe(true);
        ...
        it("t385 AC-003: postMessage 失败按快照恢复游标（轮前值保留），下轮续推不重发已入列前缀", () => {
            ...
            configure(base_config);
            // 首轮成功：截断，游标 = {s0..s9999}。
            expect(source_cursors.get("claude_costs_win")?.sessions.size).toBe(10000);
        ...
        it("t393 AC-002: daily 截断游标跨轮推进（对齐 session 维度截断用例）", () => {
            ...
            const cursor = source_cursors.get("claude_jsonl_win");
            ...
            expect(source_cursors.has("claude_jsonl_win")).toBe(false);
    ...
    describe("source status visibility (t309)", () => {
        ...
        it("AC-002/AC-003: a throwing reader is marked failed with the error and warns once", () => {
            ...
            mock_read_costs.mockImplementation((_path: string, env: string) => {
                if (env === "win") throw new Error("file locked");
            ...
            const failed = update.sources_status.find(
                (s) => s.source === "claude_code" && s.env === "win",
            );
            ...
            expect(warns[0]?.message).toContain("claude_costs_win");
    ...
    describe("bounded memory (t346)", () => {
        it("prunes emitted records older than the window (t346 AC-001)", () => {
            const now = Date.now();
            // 注入旧（>7d）与新的 key。
            emitted_record_keys.set("old|win|m-old", now - 31 * 24 * 60 * 60 * 1000);
            emitted_record_keys.set("fresh|linux|m-fresh", now);

            // 无数据 collect 也触发 prune_emitted。
            configure(base_config);

            expect(emitted_record_keys.has("old|win|m-old")).toBe(false);
            expect(emitted_record_keys.has("fresh|linux|m-fresh")).toBe(true);
        });
        ...
        it("newly emitted records carry a timestamp and are bounded by the window", () => {
            ...
            expect(emitted_record_keys.has("claude_code|win|s1|m1")).toBe(true);
            expect(emitted_record_keys.has("old|win|m-old")).toBe(false);
        });
        ...
        it("t386 AC-001: 活跃长会话 key 过窗但会话活跃 → 保留（不整段重发）", () => {
            ...
            emitted_record_keys.set(
                "claude_code|win|sess-active|m-old",
            ...
            expect(emitted_record_keys.has("claude_code|win|sess-active|m-old")).toBe(true);
            // touch 键含 source|env 前缀（生产刷新路径设）。
            expect(session_touch_ts.get("claude_code|win|sess-active")).toBeGreaterThan(now - 1000);
        });

        it("t386 AC-002: 非活跃会话 key 过窗删除，重触碰时整段重发（语义不变）", () => {
            const now = Date.now();
            emitted_record_keys.set(
                "claude_code|win|sess-idle|m-old",
                now - 31 * 24 * 60 * 60 * 1000,
            );
            ...
            configure(base_config);
            collect();
            expect(emitted_record_keys.has("claude_code|win|sess-idle|m-old")).toBe(false);
        });
        ...
        it("t386 AC-003: 同一 message_id 跨会话不误合并（key 含会话维度独立）", () => {
            ...
            expect(emitted_record_keys.has("claude_code|win|sa|dup")).toBe(true);
            expect(emitted_record_keys.has("claude_code|win|sb|dup")).toBe(true);
        });

        it("t393 AC-003: 边界到期 key 本轮即回收（去重判定前移，不延迟一轮重发）", () => {
            const now = Date.now();
            // 恰过窗口边界的 key（>30 天 1ms），会话不活跃（无 touch）。
            emitted_record_keys.set("claude_code|win|s1|m1", now - EMITTED_WINDOW_MS - 1);
            mock_scan_jsonls.mockReturnValue({
                sessions: [],
                daily: [],
    ...
    describe("t438: linux 宿主 Windows 侧源（env=win，hosts=[linux]）", () => {
        it("发现成功 → win 五源参与采集，路径基于发现 home（POSIX），status=ok", () => {
            set_collector_host("linux");
            set_win_home_wsl_probe(() => "/mnt/c/Users/Karson");
            mock_scan_kimi.mockReturnValue({
            ...
            expect(String(kimi_call![0])).toBe("/mnt/c/Users/Karson/.kimi-code/sessions");
        ...
        it("发现成功 → claude_costs/claude_jsonl/opencode/grok win 源也读取（AC-006）", () => {
            ...
            expect(String(costs_call![0])).toBe("/mnt/c/Users/Karson/.claude/metrics/costs.jsonl");
            const jsonl_call = mock_scan_jsonls.mock.calls.find((c: unknown[]) => c[1] === "win");
            expect(String(jsonl_call![0])).toBe("/mnt/c/Users/Karson/.claude/projects");
            ...
            expect(String(grok_call![0])).toBe("/mnt/c/Users/Karson/.grok/sessions");
        });
        ...
        it("发现结果进程内缓存：一轮内只探测一次（对齐 effective_wsl_user 模式）", () => {
            set_collector_host("linux");
            const probe = vi.fn(() => "/mnt/c/Users/Karson");
