...
function fixture(): string {
    return [
        line({ timestamp: "2026-09-03T17:51:43.486Z", ordinal: 0, type: "session_meta", payload: { session_id: SID, cwd: "/home/testuser/proj" } }),
        line({ timestamp: "2026-09-03T17:51:43.491Z", ordinal: 1, type: "response_item", payload: { type: "message", role: "developer", content: [{ type: "input_text", text: "sys" }] } }),
        line({ timestamp: "2026-09-03T17:51:44.000Z", ordinal: 2, type: "response_item", payload: { type: "message", role: "user", content: [{ type: "input_text", text: "hello codex" }] } }),
        line({ timestamp: "2026-09-03T17:51:45.000Z", ordinal: 3, type: "response_item", payload: { type: "reasoning", text: "thinking" } }),
        line({ timestamp: "2026-09-03T17:51:46.000Z", ordinal: 4, type: "response_item", payload: { type: "message", role: "assistant", content: [{ type: "output_text", text: "hi there" }] } }),
        line({ timestamp: "2026-09-03T17:51:47.000Z", ordinal: 5, type: "response_item", payload: { type: "function_call", name: "x", arguments: "{}" } }),
        line({ timestamp: "2026-09-03T17:51:48.000Z", ordinal: 6, type: "event_msg", payload: { type: "token_count", info: {} } }),
        line({ timestamp: "2026-09-03T17:51:49.000Z", ordinal: 7, type: "response_item", payload: { type: "web_search_call", query: "q" } }),
        "not-json{{{",
    ].join("\n");
}
...
describe("codex extractor (t446)", () => {
    ...
    it("AC-002: 追加新行后增量只返回新增", () => {
        ...
        try {
            ...
            appendFileSync(
                file,
                "\n" + line({ timestamp: "2026-09-03T17:51:50.000Z", ordinal: 8, type: "response_item", payload: { type: "message", role: "assistant", content: [{ type: "output_text", text: "second" }] } }),
            );
    ...
    it("AC-004: locator 按 session_id 解析到 rollout 文件；不存在返回 null", () => {
        ...
        try {
            ...
            expect(found?.file_path.endsWith(`rollout-2026-09-03T17-51-32-${SID}.jsonl`)).toBe(true);
            const missing = resolve_session_file("codex", "linux", "bbbbbbbb-bbbb-cccc-dddd-eeeeeeeeeeee", {
                host: "linux",
                homedir: dir,
                win_home: "",
                wsl_distro: "Ubuntu-22.04",
    ...
    it("AC-005: 大信封 user 行经归一后不展示信封原文", () => {
        ...
        const norm = normalize_user_display_text("<environment_context>\n  <cwd>/home/testuser</cwd>\n</environment_context>\nhello");
