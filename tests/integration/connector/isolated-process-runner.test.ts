...
describe("AC-002: 独立进程隔离执行器健壮性", () => {
    ...
    describe("AC-001 & AC-002: 打包配置与 worker 路径解析门禁", () => {
        it("ensures electron.vite.config.ts includes connector-worker entry", async () => {
        ...
        it("resolve_connector_worker_path resolves valid worker path", async () => {
            ...
            expect(resolved.endsWith("connector-worker.js") || resolved.endsWith(".ts")).toBe(true);
        });
    });
});
