 ...
 * - AC1/AC2：调整 agent/session 窗口位置大小 → 关闭重开 → 恢复 bounds；两窗口互不影响。
 * - AC4：无保存键时按默认尺寸。
 * - t262：session 窗口 bounds 保存 → 关闭 → 重开恢复独立用例。
 */
...
test.describe("panel window bounds persist (t251)", () => {
    ...
    test("session 窗口调整大小后重开恢复 bounds（t262）", async ({ omni }) => {
        const page = await omni.app.firstWindow();
        // 打开 session 窗口（会话面板，无会话数据亦可创建）。
        await page.evaluate(() => {
            void window.usageboard.sessionHistory.open("", "", "");
        });
        await expect(async () => {
            const b = await get_window_bounds(omni.app, "session");
            expect(b).toBeTruthy();
        }).toPass({ timeout: 10_000 });

        // 调整位置与大小（不触发 min 钳制）。
        const target = { x: 90, y: 60, width: 960, height: 680 };
        await set_window_bounds(omni.app, "session", target);
        await new Promise((r) => setTimeout(r, 800));

        // 关闭再重开。
        await app_close_session(omni.app);
        await new Promise((r) => setTimeout(r, 300));
        await page.evaluate(() => {
            void window.usageboard.sessionHistory.open("", "", "");
        });
        ...
        const restored = await get_window_bounds(omni.app, "session");
        expect(restored.x).toBe(target.x);
        expect(restored.y).toBe(target.y);
        expect(restored.width).toBeGreaterThanOrEqual(target.width - 2);
        expect(restored.height).toBeGreaterThanOrEqual(target.height - 2);
...
async function app_close_session(app: ElectronApplication): Promise<void> {
    await app.evaluate(({ BrowserWindow }) => {
        const w = BrowserWindow.getAllWindows().find((win) => {
            try {
                return win.webContents.getURL().includes("#session");
            } catch {
                return false;
            }
        });
