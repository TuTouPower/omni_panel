 ...
 * 可触发 [data-testid="card-state"][data-variant="auth"]（is_auth_error 文本匹配），留 electron 不迁。
 *
 * 保留 case：
 ...
 *    -> overview 渲染 [data-testid="card-state"][data-variant="err"] + 重试 action（错误 banner 分支）。
...
test.describe("popup card states (web)", () => {
    test("stale error banner shows retry action", async ({ webPage }) => {
        ...
        const err_banner = live
            .locator('[data-testid="card-state"][data-variant="err"]')
            .filter({ hasText: "重试" });
